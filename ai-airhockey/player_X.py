...
class Player:
    ...
    def next_move(self, current_state):
        ...
        self.lower_extreme = (current_state['board_shape'][0] / 2) - ((current_state['board_shape'][0] * current_state['goal_size']) / 2)
        self.higher_extreme = (current_state['board_shape'][0] / 2) + ((current_state['board_shape'][0] * current_state['goal_size']) / 2)

        # Determine if the last shot was to the goal:
        if self.my_goal == 'left':
            if self.puck_last_x > current_state['puck_pos']['x'] and current_state['puck_pos']['y'] > self.lower_extreme and \
                    current_state['puck_pos']['y'] < self.higher_extreme and current_state['puck_pos']['x'] < (current_state['board_shape'][1] / 4) and self.puck_crossed == 0:
            ...
            if self.puck_last_x < current_state['puck_pos']['x'] and current_state['puck_pos']['y'] > self.lower_extreme and \
                    current_state['puck_pos']['y'] < self.higher_extreme and current_state['puck_pos']['x'] > ((current_state['board_shape'][1] / 4) * 3) and self.puck_crossed == 0:
    ...
    def attack(self, current_state):
        ...
        low_extreme= {'x': x, 'y': state['board_shape'][0]/2}
        
        up_extreme={'x': x, 'y':state['board_shape'][0]/2}
        ...
        if(self.my_goal=="left" and self.puck_last_x< state['puck_pos']['x'] and state['puck_pos']['x']<(state['board_shape'][1]/2)):
            if(self.my_opponent_pos['y']>(state['board_shape'][0]*0.70)):
                print("space available in the top, shoot up")
                return up_extreme
            elif(self.my_opponent_pos['y']<(state['board_shape'][0]*0.30)):
                print("space available at the bottom, shoot down")
                return low_extreme
