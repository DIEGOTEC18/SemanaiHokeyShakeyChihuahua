...
class Player:
    ...
    def attack(self, current_state):
        ...
        t_max_paddle_A= utils.distance_between_points({'x':0, 'y':self.my_opponent_pos['y']}, {'x':0, 'y': self.goal_sideA['y']})*state['paddle_max_speed']
        t_max_paddle_B=utils.distance_between_points({'x':0, 'y':self.my_opponent_pos['y']}, {'x':0, 'y': self.goal_sideB['y']})*state['paddle_max_speed']

        if(state['puck_pos']['x']<(state['board_shape'][1]/2) and state['puck_pos']['x']>self.my_paddle_pos['x'] and self.my_goal=="left"):
            print("soy izquierdo")
        ...
        elif(state['puck_pos']['x']>(state['board_shape'][1]/2) and state['puck_pos']['x']<self.my_paddle_pos['x'] and self.my_goal=="right"):
            print("soy derecho")
            if(t_maxA_puck<t_max_paddle_A):
                print("apunta al extremo inferior directo")
